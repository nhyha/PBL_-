...
try:
    from r_paragraph.ai_dataset import AiDataset
    from r_paragraph.ai_loader import AiCollator, AiCollatorTrain, show_batch
    from r_paragraph.ai_model import (LlamaForDetectAI, MistralForDetectAI,
                                      PhiForDetectAI)
    from r_paragraph.ai_optimizer import get_optimizer
...
def run_evaluation(accelerator, model, valid_dl, valid_ids):
    ...
    # aggregate --
    result_df = result_df.groupby("id").agg({"predictions": "mean", "truths": "mean"}).reset_index()
...
@hydra.main(version_base=None, config_path="../conf/r_paragraph", config_name="conf_r_paragraph")
def run_training(cfg):
    ...
    try:
        ...
        essay_df = pd.read_csv(os.path.join(data_dir, "train_essays_general.csv"))
