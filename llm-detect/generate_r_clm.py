...
def get_instruction(inputs):
    # override persona ---
    inputs['holistic_essay_score'] = 2
    inputs['grade_level'] = 12
    inputs['ell_status'] = "No"
    inputs['student_disability_status'] = "Identified as having disability"
    # ---
...
def pre_process_essay(essay_df):
    ...
    # essay_df["prompt"] = essay_df["instruction"].apply(lambda x: f"### Instruction:\n{x}\n\n### Response: ")
    return essay_df
...
def generate(cfg):
    ...
    prompts = [p for p in prompts if "car" not in p.lower()]
    # prompts = [p for p in prompts if "Task: Text dependent" in p]
    ...
    for i in range(n_examples):
        ...
        temperature = 1.0 + 1.0 * random.random()
        top_k = random.randint(128, 256)
        penalty_alpha = 0.3 + 0.3 * random.random()
        guidance_scale = 1.0 + 0.2 * random.random()
