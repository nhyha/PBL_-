...
try:
    from r_clm.ai_dataset_ps import AiDataset
...
def run_training(cfg):
    ...
    essay_df = essay_df[~essay_df['target'].isna()].copy()
    essay_df['prefix'] = essay_df['prefix'].fillna("")
    essay_df['suffix'] = essay_df['suffix'].fillna("")
    essay_df = essay_df.sample(cfg.n_examples, random_state=cfg.seed).copy()
    essay_df = essay_df.reset_index(drop=True)

    # ------- Data Split ----------------------------------------------------------------#
    # sample validation data
    rng = random.Random(cfg.seed)
    essay_df['fold'] = essay_df['target'].apply(lambda x: 'train' if rng.random() < 0.99 else 'valid')
