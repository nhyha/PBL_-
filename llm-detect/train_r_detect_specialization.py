...
def run_training(cfg):
    ...
    # ------- Data Split ----------------------------------------------------------------#
    # valid_prompt_id = 0
    # train_df = essay_df[essay_df["prompt_id"] != valid_prompt_id].copy()
    # valid_df = essay_df[essay_df["prompt_id"] == valid_prompt_id].copy()
    # n_valid = min(2048, len(valid_df))
    # valid_df = valid_df.sample(n_valid, random_state=cfg.seed).copy()
    ...
    model = PeftModel.from_pretrained(base_model, cfg.general_adapter_path)
